<%@ Page Title="院级组织管理中心" Language="C#" MasterPageFile="~/Site.master"
         AutoEventWireup="true" CodeFile="OrgAuditDept.aspx.cs" Inherits="OrgManage.Admin_OrgAuditDept" %>
...
<asp:Content ContentPlaceHolderID="MainContent" runat="server">
    <style>
        ...
        .glass-card {
            ...
            border: 2px solid rgba(255, 193, 7, 0.7) !important; /* 边框深度从0.35→0.7 */
            border-radius: 14px !important;
            box-shadow: 
                0 6px 28px rgba(0,0,0,0.45),      /* 外部深投影 - 立体感 */
                0 0 0 1px rgba(255,193,7,0.08),   /* 外层光晕边框 */
                inset 0 1px 0 rgba(255,193,7,0.3), /* 顶部高光线 - 立体感 */
                inset 0 -3px 0 rgba(0,0,0,0.4)    /* 底部内阴影 - 立体感 */
                !important;
        ...
        .glass-card:hover {
            ...
            box-shadow: 
                0 12px 40px rgba(0,0,0,0.5),      /* 悬停时投影更深 */
                0 0 0 1px rgba(255,193,7,0.15),
                0 0 24px rgba(255,193,7,0.15),     /* 金色外发光 */
        ...
        .card-header {
            border-bottom: 2px solid rgba(255,193,7,0.25) !important; /* 加深+加粗 */
            font-weight: 500;
        ...
        .table td { 
            ...
            color: rgba(255,255,255,0.9) !important; /* 明确文字颜色 */
        }
        /* ★★★ 关键修复：hover时文字颜色必须保持白色 ★★★ */
        .table-hover tbody tr:hover {
            background-color: rgba(255,193,7,0.18) !important;
            color: #fff !important; /* 强制白色 */
        }
        .table-hover tbody tr:hover td {
            color: #fff !important; /* 强制每格文字白色 */
        }
        ...
        .member-modal {
            ...
            backdrop-filter: blur(30px) !important;      /* 模糊效果保留 */
            -webkit-backdrop-filter: blur(30px);
            border: 2px solid rgba(255, 193, 7, 0.7) !important; /* 加深边框 */
            border-radius: 14px !important;
        ...
        .member-modal .modal-header {
            padding: 24px 30px;
            border-bottom: 2px solid rgba(255,193,7,0.3) !important; /* 加深 */
            display: flex;
        ...
        .member-modal .table th {
            color: rgba(255,193,7,0.85) !important;    /* 表头金色 */
    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-bold mb-0 page-header">
                <i class="bi bi-buildings me-2"></i>院级组织管理中心
            </h3>
            <span class="badge bg-warning text-dark px-3 py-2 rounded-pill">
                <i class="bi bi-shield-fill-check me-1"></i>院级管理员
            </span>
        </div>
            ...
            <div class="col-md-6 mb-4 mb-md-0">
                <div class="card h-100 glass-card">
                    <div class="card-header d-flex justify-content-between align-items-center py-3">
                        <span><i class="bi bi-hourglass-split me-2"></i>待审核院级组织申请</span>
                        <span class="badge bg-warning text-dark rounded-pill" id="badgePending"><%= pendingCount %></span>
                    </div>
            ...
            <div class="col-md-6">
                <div class="card h-100 glass-card">
                    <div class="card-header d-flex justify-content-between align-items-center py-3">
                        ...
                        <span class="badge bg-danger text-white rounded-pill" id="badgeDisband"><%= disbandCount %></span>
                    </div>
                    <div class="card-body p-4">
                        <asp:GridView ID="gvDisband" runat="server" 
                            AutoGenerateColumns="false"
                            CssClass="table table-hover table-bordered text-center align-middle"
                            DataKeyNames="RequestID"
                            EmptyDataText=" "
                            OnRowDataBound="gvDisband_RowDataBound">
                            <Columns>
        ...
        <div class="card glass-card">
            <div class="card-header d-flex justify-content-between align-items-center py-3">
                <span><i class="bi bi-list-check me-2"></i>院级组织 & 社团</span>
                <div>
                    <!-- 【新增功能】搜索框 -->
                ...
                <asp:GridView ID="gvOrgList" runat="server" 
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="成员">
                            <ItemTemplate>
                                <%-- 【问题2修复】改为客户端按钮，通过AJAX调用WebMethod获取数据 --%>
    ...
    <asp:Button ID="btnDisbandSubmit" runat="server" OnClick="DoDisbandAudit" style="display:none" />
    ...
    <script type="text/javascript">
        ...
        function loadMembers(orgId, orgName) {
            ...
            if (typeof PageMethods !== 'undefined' && PageMethods.GetOrgMembers) {
                PageMethods.GetOrgMembers(orgId, function (result) {
                    ...
                    document.getElementById('memberLoading').style.display = 'none';
                    showToast('加载成员数据失败: ' + error.get_message(), false);
                    document.getElementById('memberEmptyState').style.display = 'block';
                    document.getElementById('memberEmptyState').querySelector('p').textContent = '加载失败，请重试';
                });
            } else {
                // 降级方案：如果PageMethods不可用，使用传统PostBack
                document.getElementById('memberLoading').style.display = 'none';
                showToast('AJAX组件不可用，使用传统方式加载', false);
        ...
        // ===== 审核操作（保留原有PostBack方式，因为涉及数据库更新） =====
        function Approve(id) {
            if (confirm("确定通过此组织申请？")) {
        ...
        function confirmDelete() {
            ...
            if (confirm("是否确定删除？一旦删除，所有关联数据将无法恢复！")) {
                closeDeleteModal();
        ...
        function filterOrgTable(e) {
            ...
            for (var i = 1; i < rows.length; i++) {
                var cell = rows[i].getElementsByTagName('td')[1]; // 组织名称列
                if (cell) {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // 确保搜索框正常显示
            var searchInput = document.getElementById('searchOrg');
            if (searchInput) {
